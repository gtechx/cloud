...
func (decoder *PhpDecoder) decodeArray() (value map[string]interface{}, err error) {
			...
			for i := 0; i < arrLen; i++ {
				if k, _err := decoder.DecodeValue(); _err != nil {
					...
					case string:
						value[k.(string)] = v
					case int64:
						s := strconv.Itoa(int(k.(int64)))
						value[s] = v
					case int:
						s := strconv.Itoa(k.(int))
						value[s] = v
					case float64:
						s := floatToString(k.(float64))
						value[s] = v
					}
				}
			}
